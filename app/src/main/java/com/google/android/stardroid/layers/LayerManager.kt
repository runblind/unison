...
class LayerManager(private val sharedPreferences: SharedPreferences) : OnSharedPreferenceChangeListener {
    ...
    /**
     * Return all the objects in the skymap
     * */
    fun getSearchIndexs(): ArrayList<ArrayList<SearchResult>>{
        var searchIndexList = ArrayList<ArrayList<SearchResult>>()
        for(layer in layers) {
            if(isLayerVisible(layer)){
                var listResults = layer.getArraySearch()
                searchIndexList.add(listResults)
            }
        }
    ...
    fun searchByObjectNameForInfos(name: String): List<SearchResult?> {
        val all: MutableList<SearchResult?> = ArrayList()
        for (layer in layers) {
            all.addAll(layer.searchByObjectName(name))
        }
