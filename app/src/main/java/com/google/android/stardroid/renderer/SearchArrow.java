...
public class SearchArrow {
  ...
  public void draw(GL10 gl, Vector3 lookDir, Vector3 upDir, SearchHelper searchHelper,
    ...
    // Positive diffTheta means you need to look left.
    ...
    //Log.e("diff", ""+(diffPhi*RADIANS_TO_DEGREES)+" : "+(diffTheta * RADIANS_TO_DEGREES));
    ...
    this.Bth.setThetaM(new float[]{diffTheta, 0f, 0f, 0f});
    this.Bth.setPhiM(new float[]{diffPhi, 0f, 0f, 0f});
    if(diffTheta > -0.2 && diffTheta < 0.2) {
      this.Bth.setAzimuthFound(true);
      if (diffPhi < -0.4) {
        this.Bth.setLookDown(true);
      } else if (diffPhi > 0.4) {
        this.Bth.setLookUp(true);
      }
    }
    else {
      this.Bth.setAzimuthFound(false);
      this.Bth.setLookUp(false);
      this.Bth.setLookDown(false);
    }
    ...
    angle += roll;
    //Log.e("angle", ""+angle);
    //this.Bth.setPanningValue(angle);
    ...
    this.Bth.setTargetFound(target);
  }
