...
public class SkyRenderer implements GLSurfaceView.Renderer {
  ...
  public String solarSystem_voice = "fr-fr-x-frd-local";
  public String stars_voice = "fr-fr-x-vlf-local";
  public String  constellations_voice = "fr-fr-x-frb-local";

  private boolean modeEasyVoice = true;

  private TextToSpeech tts;

  public ArrayList<SearchResult> closeObjects = new ArrayList<>();

  private boolean find = false;

  public Float viewDirectionX;
  public Float viewDirectionY;
  public Float viewDirectionZ;

  private ArrayList<ArrayList<SearchResult>> starsObject;

  private ArrayList<Boolean> buttonStates;
  private int count = 0;
  ...
  public void setViewOrientation(float dirX, float dirY, float dirZ,
    ...
    this.viewDirectionX = dirX;
    this.viewDirectionY = dirY;
    this.viewDirectionZ = dirZ;
  ...
  public void closeObjectsWithPosition(){
    //String result = "";
    ArrayList<SearchResult> objects = new ArrayList<>();
    for(ArrayList<SearchResult> list:this.starsObject){
      objects.addAll(list);
    }
    //Log.e("objects", objects.toString());
    closeObjects.clear();
    closeObjects = getCloseObjects(objects,viewDirectionX, viewDirectionY, viewDirectionZ);
    new Handler(Looper.getMainLooper()).postDelayed(this::closeObjectsWithPosition, 3000);
  }

  private double distancePositionObject(float dirX, float dirY, float dirZ, float objectX, float objectY, float objectZ) {
    return Math.sqrt(Math.pow((dirX-objectX), 2.0)+Math.pow((dirY-objectY), 2.0)+Math.pow((dirZ-objectZ), 2.0));
  }

  public void readCloseObjects(){
    if(!tts.isSpeaking()){
      ArrayList<SearchResult> etoiles = new ArrayList<>();
      ArrayList<SearchResult> constellations = new ArrayList<>();
      ArrayList<SearchResult> solarSystem = new ArrayList<>();
      if(buttonStates.get(0) && !buttonStates.get(1)){
        etoiles.addAll(closeObjects);
      }
      else if(buttonStates.get(1) && !buttonStates.get(0)){
        constellations.addAll(closeObjects);
      }
      else if(buttonStates.get(2) && !buttonStates.get(0)){
        solarSystem.addAll(closeObjects);
      }
      else {
        for (SearchResult search : closeObjects) {
          if (this.starsObject.get(0).contains(search)) {
            Log.e("etoiles", search.toString());
            etoiles.add(search);
          } else if (this.starsObject.get(1).contains(search)) {
            Log.e("constellations", search.toString());
            constellations.add(search);
          } else {
            Log.e("systeme solaire", search.toString());
            solarSystem.add(search);
          }
        }
      }
      //Log.e("result", ""+result);
      Log.e("etoiles", etoiles.toString());
      Log.e("constellations", constellations.toString());
      Log.e("systeme solaire", solarSystem.toString());
      speakCloseObjects(etoiles, constellations, solarSystem);
    }
  }

  private void speakCloseObjects(ArrayList<SearchResult> etoiles,
                                  ArrayList<SearchResult> constellations,
                                  ArrayList<SearchResult> solarSystem){
    if(!etoiles.isEmpty() || !constellations.isEmpty() || !solarSystem.isEmpty()) {
      if (!tts.isSpeaking()) {
        if (!etoiles.isEmpty()) {
          setVoiceToSpeech(stars_voice);
          if(modeEasyVoice){
            speakOut("Étoiles: " + etoiles);
          }
          else{
            speakOut(etoiles.toString());
          }
          etoiles.clear();
        } else if (!constellations.isEmpty()) {
          setVoiceToSpeech(constellations_voice);
          if(modeEasyVoice){
            speakOut("Constellations: " + constellations);
          }
          else{
            speakOut(constellations.toString());
          }
          constellations.clear();
        } else if (!solarSystem.isEmpty()) {
          setVoiceToSpeech(solarSystem_voice);
          if(modeEasyVoice){
            speakOut("Éléments du système solaire: " + solarSystem);
          }
          else{
            speakOut(solarSystem.toString());
          }
          solarSystem.clear();
        }
      }
  ...
  private ArrayList<SearchResult> getCloseObjects(ArrayList<SearchResult> objects,
                                                   float dirX, float dirY, float dirZ){

    Map<SearchResult, Double> distances = new HashMap<>();
    ArrayList<SearchResult> newResult = new ArrayList<>();
    for(int i = 0; i < objects.size(); i++){
      SearchResult object = objects.get(i);
      distances.put(object, distancePositionObject(dirX, dirY, dirZ, object.coords().x, object.coords().y, object.coords().z));
    }
    List<Map.Entry<SearchResult, Double>> list = new ArrayList<>(distances.entrySet());
    list.sort(Map.Entry.comparingByValue());
    for (int i =0; i < 5; i++) {
      newResult.add(list.get(i).getKey());
    }
    //Log.e("closeObjects", newResult.toString());
    return newResult;
  }
  private String getObjectsInDir(ArrayList<SearchResult> list, float dirX, float dirY, float dirZ){
    String result = "";
    for(int i = 0; i < list.size(); i++){
      if(list.get(i).coords().x < dirX +0.3 && list.get(i).coords().x > dirX - 0.3){
        if(list.get(i).coords().y < dirY +0.3 && list.get(i).coords().y > dirY - 0.3){
          if(list.get(i).coords().z < dirZ +0.3 && list.get(i).coords().z > dirZ - 0.3){
  ...
  /**
   * Set the arraylist with data
   * */
  public void setStarsObject(ArrayList<ArrayList<SearchResult>> starsObject) {
    this.starsObject = starsObject;
  }

  /**
   * Set the state of the buttons for stars, constellations and planets
   * */
  public void setButtonStates(ArrayList<Boolean> buttonStates){
    this.buttonStates = buttonStates;
  }

  /**
   * Setup the TextToSpeach
   * */
  ...
  /**
   * Check which CSV files are needed to read
   * */
  public void checkCSV(Context context, String objectname){
    find = false;
    //if(this.buttonStates.get(0)){//étoiles
      readCSV(context, objectname, "stars_data.csv");
    //}
    new Handler(Looper.getMainLooper()).postDelayed(() -> {
      if(!find){//constellations
        readCSV(context, objectname, "constellations_data.csv");
      }
    }, 100);
    new Handler(Looper.getMainLooper()).postDelayed(() -> {
      if(!find){//planètes
        readCSV(context, objectname, "solarSystem_data.csv");
      }
    }, 100);
    if(!find){
      speakOut("élément introuvable");
    }
    find = false;
  }
  ...
  /**
   * Read CSV to give infos about specific object
   * */
  public String[] readCSV(Context context, String objectName, String fileName) {
    ...
    try {
      ...
      while ((nextLine = csvReader.readNext()) != null) {
        // nextLine est un tableau contenant les données de chaque ligne
        if(fileName.contains("keyword")){
          //for(String word : nextLine[1])
          String[] words = nextLine[1].split(",");
          for(String word:words){
            if(word.contains(objectName.toLowerCase())){
              result = nextLine;
              break;
            }
          }
          ...
          String name = nextLine[0];        // Nom de l'étoile
          Log.e("name", name + " : " + objectName);
          if (Objects.equals(name.toLowerCase(), objectName.toLowerCase())){
            actionFromCSVLineToStarsObjects(nextLine, fileName);
            //Log.e("CSV result", result);
            result = nextLine;
            break;
          }
        }
  ...
  private void actionFromCSVLineToStarsObjects(String[] nextLine, String fileName){
    if (!fileName.contains("select")) {
      String infos = nextLine[1];
      if (fileName.contains("stars")) {
        setVoiceToSpeech(stars_voice);
      } else if (fileName.contains("constellations")) {
        setVoiceToSpeech(constellations_voice);
      } else if (fileName.contains("solarSystem")) {
        setVoiceToSpeech(solarSystem_voice);
      }
      speakOut(infos);
      find = true;
  ...
  /**
   * Give infos about specific object
   * */
  @NonNull
  /*private static String getInfoString(String fileName, String name, String[] nextLine) {
    return result;
  }*/

  public void setOverlayBluetooth(Bluetooth bluetooth){
    this.mOverlayManager.setArrowBluetooth(bluetooth);
  }

  public void setModeEasyVoice(boolean mode){
    modeEasyVoice = mode;
  }

  public boolean getModeEasyVoice(){
    return modeEasyVoice;
  }

  public ArrayList<ArrayList<SearchResult>> getStarsObjects(){
